...
public class FairFareEngine {
	...
	private void loadFareData() {
		/*
		 * Get the initial values from the Preferences file, default values
		 * provided in case no preferences exist.
		 */
		...
		float defaultMinFare = Float.parseFloat(res
				.getString(R.string.saved_default_minFare));
		float defaultMinDist = Float.parseFloat(res
				.getString(R.string.saved_default_minDist));
		float defaultFarePerKm = Float.parseFloat(res
				.getString(R.string.saved_default_farePerKm));

		minFare = sharedPref.getFloat(res.getString(R.string.saved_minFare),
				defaultMinFare);
		minDist = sharedPref.getFloat(res.getString(R.string.saved_minDist),
				defaultMinDist);
		farePerKm = sharedPref.getFloat(
				res.getString(R.string.saved_farePerKm), defaultFarePerKm);
	}

	public void resetToDefaultFareData(FragmentActivity activity) {
		/*
		 * Update preferences and class values to defaults as defined in
		 * string.xml saved_default_* strings
		 */
		SharedPreferences sharedPref = activity.getSharedPreferences(
				activity.getString(R.string.preference_file_key),
				Context.MODE_PRIVATE);
		...
		minFare = Float.parseFloat(res
				.getString(R.string.saved_default_minFare));
		minDist = Float.parseFloat(res
				.getString(R.string.saved_default_minDist));
		farePerKm = Float.parseFloat(res
				.getString(R.string.saved_default_farePerKm));
	...
	public void storeFareData(FragmentActivity activity, float minFare,
			float minDist, float farePerKm) {
		/*
	...
	public void findMyLocation(Location myLocation) {
		...
		if (myLocation != null) {
			...
			CameraPosition cameraPosition = new CameraPosition.Builder()
					.target(convertLocationToLatLng(myLocation)) // Sets the
																	// center of
																	// the map
																	// to
																	// location
	...
	void setMarkers(DirectionsJSONParser parser) {
		...
		srcOpt.icon(BitmapDescriptorFactory
				.defaultMarker(BitmapDescriptorFactory.HUE_GREEN));
		srcOpt.visible(true);
		srcOpt.position(src);
		...
		destOpt.icon(BitmapDescriptorFactory
				.defaultMarker(BitmapDescriptorFactory.HUE_RED));
	...
	public float getFare(float dist) {
		/*
		 * Returns Fare=minFare (For upto minDist km) Returns Fare=minFare +
		 * farePerKm *(dist-minDist) (for journeys greater than minDist km
		 */
	...
	public void getDataAndProcess(String origin, String dest) {
		activity.getSupportFragmentManager().beginTransaction().replace(R.id.container,new MainActivity.LoadingFragment()).addToBackStack(null).commit();
	...
	private class ParserTask extends
			AsyncTask<String, Integer, DirectionsJSONParser> {
		...
		protected void onPostExecute(DirectionsJSONParser parsedResult) {
			...
			if (parsedResult == null) {
				...
				ErrorFragment newFragment = ErrorFragment.newInstance(cause,
						help);
			...
			if (routes == null || routes.size() < 1) {
				...
				Toast.makeText(activity.getBaseContext(),
						"No Points. Try Changing your query..",
			...
			for (int i = 0; i < routes.size(); i++) {
				...
				for (int j = 0; j < path.size(); j++) {
					...
					if (j == 0) { // Get distance from the list
						continue;
					} else if (j == 1) { // Get distanceval from the list
						continue;
					}
	...
	void postProcess(DirectionsJSONParser parsedResult,
		...
		map.moveCamera(CameraUpdateFactory.newLatLngBounds(new LatLngBounds(
				parsedResult.getSwBound().get(0), parsedResult.getNeBound()
						.get(0)), 20));
